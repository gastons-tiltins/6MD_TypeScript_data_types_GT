...
// const products:string[] = ['Course', 'not Course', 'something'];

function filterCourses(products: Product[]) {
    return products.filter((product) => product.type === 'Course');
}

function getTitles(products: Product[]) /* add type annotation here */ {
...
const titles2 = getTitles(products);

// Exercise 4,
// Can I pass a Product object to the format Price function without
// typescript error? Why?

// Answer:     If two objects have the same shape, they are considered to be of the same type.
...
// here.
// Create a proper type definition based on the usage of the product,
// correct the input data and the function usage below based on that.

// No uzdevuma nav skaidrs, ko tieshi drikst mainit pie function `input` un pie `usage` un ko nedrikst.
...
// Change the Course interface to conform all possible formats.
...
//  Ooops, after the Course interface is changed,
...
//  fill the Type Alias for the account object
...
// Interesting, here we are not using the Account Type Alias,
//  however the function is correctly typed, and accepts accounts.
//  Why?
// Because it has same structure as type above.
...
//  The fetch account method just fetch a general object,
...
const fetchAccount = (id: number): object => ({
    id: id,
    name: 'Some Account',
    currency: 'USD',
});
const currentAccount: {id?: number; name?: string; currency?: string} =
    fetchAccount(4); /* add Type Assertion here */
...
type USD = 'USD';
type EUR = 'EUR';
//  Correct the Currency type, to accept
...
//  When corrected the Currency type, another issue come up
//  later in the code.
...
//  correct AccountWithOrWithoutCurrency or
